...
function HostPropertyView({ panel, target }: any) {
  ...
  useEffect(() => {
    ...
    setRawContent(contentLines.map((line, index) => {
      ...
      <ReflexContainer orientation="horizontal">
        <ReflexElement className="c-host-property-view__properties">
          ...
          <section>
            ...
            <table>
              <tbody>
              { host.state.health.components.map((component: any) => {
                ...
                return [
                  <tr key={component.instanceId}>
                    ...
                    <td key={`${component.instanceId}-updated`}>{ moment(component.updated).format('DD-MM-YYYY HH:mm:ss') }</td>
                    <td key={`${component.instanceId}-version`}>{ version }</td>
                    <td key={`${component.instanceId}-status`}><Status type="health" status={component.status} /></td>
                  </tr>,

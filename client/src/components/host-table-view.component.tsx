          ...
          {(() => {
            if (hosts && hosts.length) {
              return hosts.map((host: any) => {
                const rundown =
                  host.state.rundown && host.state.rundown.actives && host.state.rundown.actives.length > 0
                    ? host.state.rundown.actives[0]
                    : {}
                const rundownStatus =
                  host.rundown === null
                    ? 'Loading...'
                ...
                return (
                  <tr
                    key={host.name}
                    className={selectedHosts.map(({ host: name }: any) => name).includes(host.name) ? 'current' : ''}
                    onClick={(event) => onClick(event, host)}
                  >
                    <td key={`${host.name}-name.`}>{host.name}</td>
                    <td key={`${host.name}-updated.`}>
                      {moment(host.state.health.updated).format('DD-MM-YYYY HH:mm:ss')}
                    </td>

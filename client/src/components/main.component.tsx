      ...
      <ReflexContainer orientation={splitOrientation}>
        {[
          <ReflexElement key="host-view" className="left-pane">
            <HostTableView
              selectedHosts={hosts.property}
              hosts={hosts.display.map((name: string) => ({ name, state: hosts.raw[name] }))}
              onClick={handleHostTableViewClick}
            />
          </ReflexElement>,
          ...
          showDocumentation && (
            <ReflexElement key="documentation-view">

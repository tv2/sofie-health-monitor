...
const reducer = (state: any = initialState, action: any) => {
  switch (action.type) {
    ...
    case ActionType.SET_HOST: return { ...state,
      raw: { ...state.raw,
        [action.payload.name]: action.payload.state,
      },
    }

    case ActionType.SET_DISPLAY_HOSTS: return { ...state,
      display: action.payload,
    }

    case ActionType.SET_PROPERTY_HOST:
    return { ...state,
      property: setPropertyHost(action.payload, state.property)
    }

    case ActionType.REMOVE_PROPERTY_HOST:
      return { ...state,
        property: state.property.filter((prop: any) => prop.panel !== action.payload.panel)
    }

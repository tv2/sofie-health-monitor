import {
  ...
  AbstractSyntaxTreeOption as ASTOpt,
  AbstractSyntaxTreeOperator as ASTOpe,
} from './tokens'
...
export const interpret =
  ...
  (host: any): boolean => {
    switch (query.type) {
      case 'operator':
        return interpretOperator(query, host)
      case 'option':
        return interpretOption(query, host)
    }
  }
...
function interpretOption({ key, value }: ASTOpt, host: any): boolean {
  switch (key) {
    case 's':
    case 'status':
      return interpretOptionStatus(value, host)
    case 'r':
    case 'rundown':
      return interpretOptionRundown(value, host)
    case 'n':
    case 'name':
      return interpretOptionName(value, host)
    case 'is':
      return interpretOptionIs(value, host)
    default:
...
function interpretOperator({ key, left, right }: ASTOpe, host: any): boolean {
  switch (key) {
    case 'and':
      return interpretOperatorAnd(left, right, host)
    case 'or':
      return interpretOperatorOr(left, right, host)
    case '!':
      return interpretOperatorNot(left, right, host)
    default:

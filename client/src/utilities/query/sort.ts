...
const extractHostData = (options: QuerySortOption) => (host: any) => {
  switch (options.strategy) {
    case 'status':
      return rateStatus(host.state.health.status)
    case 'rundown':
      return rateRundown(getRundown(host))
    case 'name':
      return {
...
const sortHostData = (options: QuerySortOption) => (data1: any, data2: any) => {
  switch (options.strategy) {
    case 'name':
      return data1.str == data2.str ? (data1.dig > data2.dig ? 1 : -1) : data1.str > data2.str ? 1 : -1
    default:
